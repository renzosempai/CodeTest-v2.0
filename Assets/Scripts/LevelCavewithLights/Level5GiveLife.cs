...
public class Level5GiveLife : MonoBehaviour {
	...
	public GameObject CallLevel5GiveLife;
	...
	void Update () {
		...
		if (choiceSelected == "y") {
			...
			if (correctAnswer[randQuestion] != selectedAnswer) {
				...
				GameObject.Find ("Guess").GetComponent<Level5GiveLifeGuessTracker> ().guesscount--;
				//				StartCoroutine(Wait());
	...
	public void WrongDone(){
		...
		GameObject.Find ("Cole").GetComponent<InteractTrainer> ().defeated = true;
		GameObject.Find ("Cole").GetComponent<InteractTrainer> ().busy = false;
		CallLevel5GiveLife.SetActive (false);
