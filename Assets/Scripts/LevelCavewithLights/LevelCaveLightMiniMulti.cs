...
public class LevelCaveLightMiniMulti : MonoBehaviour {
	...
	public GameObject CallLevelCaveLightMiniMulti;
	...
	void Update () {
		...
		if (choiceSelected == "y") {
			...
			if (correctAnswer[randQuestion] != selectedAnswer) {
				...
				GameObject.Find ("Guess").GetComponent<LevelCaveLightGuessTracker> ().guesscount--;
				//				StartCoroutine(Wait());
	...
	public void MiniDone(){
		...
		GameObject.Find ("Abra").GetComponent<InteractTrainer> ().defeated = true;
		GameObject.Find ("Abra").GetComponent<InteractTrainer> ().busy = false;
		/* StartCoroutine(BacktoWorld()); */
		CallLevelCaveLightMiniMulti.SetActive (false);
