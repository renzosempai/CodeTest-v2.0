...
public class LevelCaveLightMiniMulti1 : MonoBehaviour {
	...
	public GameObject CallLevelCaveLightMiniMulti1;
	...
	void Update () {
		...
		if (choiceSelected == "y") {
			...
			if (correctAnswer[randQuestion] != selectedAnswer) {
				...
				GameObject.Find ("Guess").GetComponent<LevelCaveLightGuessTracker1> ().guesscount--;
				//				StartCoroutine(Wait());
	...
	public void MiniDone(){
		...
		GameObject.Find ("Keziah").GetComponent<InteractTrainer> ().defeated = true;
		GameObject.Find ("Keziah").GetComponent<InteractTrainer> ().busy = false;
		/* StartCoroutine(BacktoWorld()); */
		CallLevelCaveLightMiniMulti1.SetActive (false);
