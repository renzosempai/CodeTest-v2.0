...
public class Level4TrueOrFalse : MonoBehaviour {

	List<string> questions = new List<string> () {" "};
	List<string> correctAnswer = new List<string>() {" "};
	public GameObject CallLevel4TrueOrFalse;
...
//			GetComponent<Text> ().text = questions [randQuestion];
	...
	public void Done(){
		...
		GameObject.Find ("Marshall").GetComponent<InteractTrainer> ().defeated = true;
		GameObject.Find ("Marshall").GetComponent<InteractTrainer> ().busy = false;
	/* 	StartCoroutine(BacktoWorld()); */
		CallLevel4TrueOrFalse.SetActive (false);
