...
public class Level4GiveLife : MonoBehaviour {
	...
	public GameObject CallLevel4GiveLife;
	...
	void Update () {
		...
		if (choiceSelected == "y") {
			...
			if (correctAnswer[randQuestion] != selectedAnswer) {
				...
				GameObject.Find ("Guess").GetComponent<Level4GiveLifeGuessTracker> ().guesscount--;
				//				StartCoroutine(Wait());
	...
	public void WrongDone(){
		...
		GameObject.Find ("Alinity").GetComponent<InteractTrainer> ().defeated = true;
		GameObject.Find ("Alinity").GetComponent<InteractTrainer> ().busy = false;
		CallLevel4GiveLife.SetActive (false);
