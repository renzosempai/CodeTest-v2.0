...
public class Level4MiniMulti2 : MonoBehaviour {
	...
	public GameObject CallLevel4MiniMulti2;
	...
	void Update () {
		...
		if (choiceSelected == "y") {
			...
			if (correctAnswer[randQuestion] != selectedAnswer) {
				...
				GameObject.Find ("Guess").GetComponent<Level4GuessTracker2> ().guesscount--;
				//				StartCoroutine(Wait());
	...
	public void MiniDone(){
		...
		GameObject.Find ("Alona").GetComponent<InteractTrainer> ().defeated = true;
		GameObject.Find ("Alona").GetComponent<InteractTrainer> ().busy = false;
		/* StartCoroutine(BacktoWorld()); */
		CallLevel4MiniMulti2.SetActive (false);
