...
public class Level6GiveLife : MonoBehaviour {
	...
	public GameObject CallLevel6GiveLife;
	...
	void Update () {
		...
		if (choiceSelected == "y") {
			...
			if (correctAnswer[randQuestion] != selectedAnswer) {
				...
				GameObject.Find ("Guess").GetComponent<Level6GiveLifeGuessTracker> ().guesscount--;
				//				StartCoroutine(Wait());
	...
	public void WrongDone(){
		...
		GameObject.Find ("Gabriel").GetComponent<InteractTrainer> ().defeated = true;
		GameObject.Find ("Gabriel").GetComponent<InteractTrainer> ().busy = false;
		CallLevel6GiveLife.SetActive (false);
