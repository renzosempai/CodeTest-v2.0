...
public class Level6MiniMulti : MonoBehaviour {
	...
	public GameObject CallLevel6MiniMulti;
	...
	void Update () {
		...
		if (choiceSelected == "y") {
			...
			if (correctAnswer[randQuestion] != selectedAnswer) {
				...
				GameObject.Find ("Guess").GetComponent<Level6GuessTracker> ().guesscount--;
				//				StartCoroutine(Wait());
	...
	public void MiniDone(){
		...
		GameObject.Find ("Abby").GetComponent<InteractTrainer> ().defeated = true;
		GameObject.Find ("Abby").GetComponent<InteractTrainer> ().busy = false;
		/* StartCoroutine(BacktoWorld()); */
		CallLevel6MiniMulti.SetActive (false);
