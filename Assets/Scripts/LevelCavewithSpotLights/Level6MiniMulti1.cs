...
public class Level6MiniMulti1 : MonoBehaviour {
	...
	public GameObject CallLevel6MiniMulti1;
	...
	void Update () {
		...
		if (choiceSelected == "y") {
			...
			if (correctAnswer[randQuestion] != selectedAnswer) {
				...
				GameObject.Find ("Guess").GetComponent<Level6GuessTracker1> ().guesscount--;
				//				StartCoroutine(Wait());
	...
	public void MiniDone(){
		...
		GameObject.Find ("Terrence").GetComponent<InteractTrainer> ().defeated = true;
		GameObject.Find ("Terrence").GetComponent<InteractTrainer> ().busy = false;
		/* StartCoroutine(BacktoWorld()); */
		CallLevel6MiniMulti1.SetActive (false);
