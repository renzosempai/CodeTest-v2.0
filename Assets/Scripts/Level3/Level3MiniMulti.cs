...
public class Level3MiniMulti : MonoBehaviour {
	...
	public GameObject CallLevel3MiniMulti;
	...
	void Update () {
		...
		if (choiceSelected == "y") {
			...
			if (correctAnswer[randQuestion] != selectedAnswer) {
				...
				GameObject.Find ("Guess").GetComponent<Level3GuessTracker> ().guesscount--;
				//				StartCoroutine(Wait());
	...
	public void MiniDone(){
		...
		GameObject.Find ("Pawpaw").GetComponent<InteractTrainer> ().defeated = true;
		GameObject.Find ("Pawpaw").GetComponent<InteractTrainer> ().busy = false;
		/* StartCoroutine(BacktoWorld()); */
		CallLevel3MiniMulti.SetActive (false);
