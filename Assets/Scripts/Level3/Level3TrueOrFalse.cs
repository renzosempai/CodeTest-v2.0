...
public class Level3TrueOrFalse : MonoBehaviour {

	List<string> questions = new List<string> () {" "};
	List<string> correctAnswer = new List<string>() {" "};
	public GameObject CallLevel3TrueOrFalse;
...
//			GetComponent<Text> ().text = questions [randQuestion];
	...
	public void Done(){
		...
		GameObject.Find ("Gerwin").GetComponent<InteractTrainer> ().defeated = true;
		GameObject.Find ("Gerwin").GetComponent<InteractTrainer> ().busy = false;
	/* 	StartCoroutine(BacktoWorld()); */
		CallLevel3TrueOrFalse.SetActive (false);
