...
public class Level3MiniMulti1 : MonoBehaviour {
	...
	public GameObject CallLevel3MiniMulti1;
	...
	void Update () {
		...
		if (choiceSelected == "y") {
			...
			if (correctAnswer[randQuestion] != selectedAnswer) {
				...
				GameObject.Find ("Guess").GetComponent<Level3GuessTracker1> ().guesscount--;
				//				StartCoroutine(Wait());
	...
	public void MiniDone(){
		...
		GameObject.Find ("Luke").GetComponent<InteractTrainer> ().defeated = true;
		GameObject.Find ("Luke").GetComponent<InteractTrainer> ().busy = false;
		/* StartCoroutine(BacktoWorld()); */
		CallLevel3MiniMulti1.SetActive (false);
