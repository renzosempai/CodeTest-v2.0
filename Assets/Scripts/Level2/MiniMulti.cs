...
public class MiniMulti : MonoBehaviour {
	...
	public GameObject CallMiniMulti;
	...
	void Update () {
		...
		if (choiceSelected == "y") {
			...
			if (correctAnswer[randQuestion] != selectedAnswer) {
				...
				GameObject.Find ("Guess").GetComponent<Level2GuessTracker> ().guesscount--;
				//				StartCoroutine(Wait());
	...
	public void MiniDone(){
		...
		GameObject.Find ("Bonnie").GetComponent<InteractTrainer> ().defeated = true;
		GameObject.Find ("Bonnie").GetComponent<InteractTrainer> ().busy = false;
		/* StartCoroutine(BacktoWorld()); */
		CallMiniMulti.SetActive (false);
