...
public class Level2GiveLife : MonoBehaviour {
	...
	public GameObject CallLevel2GiveLife;
	...
	void Update () {
		...
		if (choiceSelected == "y") {
			...
			if (correctAnswer[randQuestion] != selectedAnswer) {
				...
				GameObject.Find ("Guess").GetComponent<Level2GuessTracker3> ().guesscount--;
				//				StartCoroutine(Wait());
	...
	public void WrongDone(){
		...
		GameObject.Find ("Mawmaw").GetComponent<InteractTrainer> ().defeated = true;
		GameObject.Find ("Mawmaw").GetComponent<InteractTrainer> ().busy = false;
		CallLevel2GiveLife.SetActive (false);
