...
public class MiniMulti1 : MonoBehaviour {
	...
	public GameObject CallMiniMulti1;
	...
	void Update () {
		...
		if (choiceSelected == "y") {
			...
			if (correctAnswer[randQuestion] != selectedAnswer) {
				...
				GameObject.Find ("Guess").GetComponent<Level2GuessTracker1> ().guesscount--;
				//				StartCoroutine(Wait());
	...
	public void MiniDone(){
		...
		GameObject.Find ("Jefferson").GetComponent<InteractTrainer> ().defeated = true;
		GameObject.Find ("Jefferson").GetComponent<InteractTrainer> ().busy = false;
		/* StartCoroutine(BacktoWorld()); */
		CallMiniMulti1.SetActive (false);
